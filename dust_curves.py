...
def powerlaw(wave, tau_v = 1, alpha = 1.3):
...
"""Calzetti et al. 2000 starburst attenuation curve"""
    ...
    ff += ( (wave >= 1200.) & (wave < 6300) )*(2.659*(-2.156+1.509*x-0.198*x^2.+0.011*x^3.)+R_v)
...
def conroy(wave, tau_v, R_v=3.1, f_bump=0.7):
...
"""Cardelli, Clayton, and Mathis Milky Way extinction curve"""
    ...
    w1 = np.where( (x >= 1.1) & (x <= 3.3) )
    w2 = np.where( (x >= x_inf) & (x < 1.1) )
    w3 = np.where( (x > 3.3) & (x <= 8) )
    w4 = np.where( (x > 8.0) & (x <= x_sup) )
...
"""As in V. Wild, i.e. power-law slope can change between regions"""
    if len(breaks) = len(alpha)+1 : print "make sure of your power law breaks"
    tau=np.array(len(wave))
    for i in range(alpha):
	inds=np.where(wave > breaks[i] and wave <=breaks[i+1])
