...
class BaSeL3(SpecLibrary):
    ...
    def generate_spectrum(self, logl, logt, logg, Z, **extras):
        ...
        spec = self.interpolate_to_pars(np.array([logt, logg, logz]).T,
                                        parnames=['LOGT', 'LOGG', 'M_H'],
                                        **extras)

        lbol = observate.Lbol(self.wavelength, spec)
        log_lbol_lsun = (np.log10(lbol) - np.log10(lsun))
    ...
    def read_all_Z(self):
        ...
        for i, f in enumerate(flist):
            ...
            logz = float(Z.replace('m', '-').replace('p', ''))
            self.Z_list.append(10**logz * 0.0190)
    ...
    def read_one_Z(self, inZ):
        ...
        if type(inZ) is float:
            ...
            Zname = inZ
            logz = float(inZ.replace('m', '-').replace('p', ''))
            inZ = 10**logz * 0.0190
        filename = glob.glob(self.specdir + 'wlbc99_' + Zname + '_cor.fits')
        parnames = ['LOGL', 'LOGG', 'LOGT', 'M_H',
                    'V_TURB', 'XH', 'ID']
...
class FSPS(SpecLibrary):
    ...
    def spectra_from_pars(self, parstruct, sps=None, **extras):
        ...
        for i, row in enumerate(parstruct):
            ...
            spec[i, :] = sps.get_starspec(m, t, l, g, 0.0, 0.0,
                                          zmet=zmet, peraa=True)
